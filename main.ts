  ...
  if (message.content.startsWith("-") && message.author.id != botId) {
    ...
    if (message.content.slice(1, 12) === "leaderboard") {
      mysql_pool.getConnection(function (err, con) {
        ...
        con.query(
          "SELECT * FROM petepuntos ORDER BY score DESC",
    ...
    if (message.content.slice(1, 5) === "pete") {
      if (message.author.id == "127586025856827392")
        //Ban a diablo
        return;
      ...
      if (message.mentions.users.first().id === "172111502612889601") {
        ...
        message.channel.send(
          "Qué le pones un petepunto a lula nabo, +1 petepunto para " +
      ...
      message.channel.send(
        "+1 petepunto para " + message.mentions.users.first().username + "."
      );
    }
    ...
    if (message.content.slice(1, 6) === "sacar") {
      if (message.author.id == "127608465525833728") {
        if (message.mentions.users.first() == undefined) {
          message.channel.send(
            "Te faltó mencionar a quién sacarle los petepuntos."
          );
          return;
        }
        ...
        if (temp > 0) {
          ...
          message.channel.send(
            "Cantidad inválida o formato inválido (!!sacar <cantidad> @<mention>)"
          );
        }
      } else {
        message.channel.send(
          "Solamente lait tiene permiso para sacar petepuntos."
        );
      }
    }
    ...
    if (message.content.slice(1, 8) === "limpiar") {
      if (
        message.author.id == "127608465525833728" ||
        message.author.id == "127584559016443904"
      ) {
        ...
        if (temp > 0 && temp <= 99) {
            ...
            await message.channel.messages
              ...
              .then((messages) => {
                message.channel
                  ...
                  .catch((error) =>
                    message.channel.send(
                      "No tengo permisos para borrar esos mensajes :pepehands:"
                    )
                  );
              });
          }
          ...
          message.channel.send(
            "Cantidad inválida o formato inválido (-limpiar <cantidad: 2-99>"
          );
        }
      } else {
        message.channel.send(
          "Solamente lait y LoaP tienen permiso para borrar mensajes."
        );
      }
    }
      ...
      if (message.content.slice(1, 6) === "mimic") {
        if (message.channel.type === "dm") {
          client.channels.cache
            .get("508818378182623242")
            .send(message.content.slice(7));
        } else {
...
function AddPoint(id, name) {
  mysql_pool.getConnection(function (err, con) {
    ...
    con.query(
      "SELECT * FROM petepuntos WHERE userid =" + id,
      function (err, result, fields) {
        if (err) console.log(err);
        if (result.length === 0) {
          con.query(
            "INSERT INTO petepuntos (userid, user, score) VALUES (" +
              id +
              ", '" +
              name +
              "', 1)"
          ...
          con.query(
            "UPDATE petepuntos SET score = score + 1 WHERE userid =" + id
          );
  ...
  mysql_pool.getConnection(function (err, con) {
    ...
    con.query(
      ...
      function (err, result, fields) {
        ...
        if (result.length === 0) {
          console.log(
            "No hice ningún cambio porque ese usuario no existe en la base de datos."
          );
        } else {
          con.query(
            "UPDATE petepuntos SET score = score - " +
              amount +
              " WHERE userid =" +
              id
          );
  ...
  mysql_pool.getConnection(function (err, con) {
    ...
    con.query(
      ...
      function (err, result, fields) {
        ...
        if (result.length === 0) {
          con.query(
            "INSERT INTO petepuntos (userid, user, credits) VALUES (" +
              id +
              ", '" +
              name +
              "', " +
          ...
          console.log(
            "Agregué " + amount + " créditos, creando un valor nuevo."
          );
        } else {
          con.query(
            "UPDATE petepuntos SET credits = credits + " +
              amount +
              " WHERE userid =" +
              id
          );
          console.log(
            "Agregué " + amount + " créditos, en un valor existente."
          );
        }
      }
    );
...
async function FinishGamble(message) {
  ...
  for (const property in pool) {
    ...
    message.channel.send(
      user.username + " rolleó " + pool[property].toString()
    );
    if (pool[property] === roll) kick = true;
  }
  const user = firstMention
    ? firstMention
    : await client.users.fetch(randomPlayer);
  if (kick) {
    //message.guild.members.get(temp).kick("Perdiste la ruleta rusa :(");
    message.channel.send(
      user.username + " perdió la ruleta rusa 👋 (+1 petepunto)"
    );
